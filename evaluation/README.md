...
선택 엔진 rank(user, 동일 holdout 영화 ID)
        ↓
공통 NDCG / tie-aware Precision / coverage 계산
        ↓
...
| `Tie-aware Precision@K` | 실제 평점순 K번째 평점을 합격선으로 삼아 Top-K 중 합격선 이상 영화의 비율 | 경계 평점 동점을 임의로 정답/오답으로 나누지 않고 Top-K 적중도를 0~1로 해석할 수 있음 | Top-K 내부의 순서와 평점 차이를 반영하지 않음 | 현재 보조 지표 |
...
**최종 선택:** `NDCG@20%`를 주 지표로, `Tie-aware Precision@20%`를 보조 지표로 사용한다. NDCG는 평점 차이와 순위 위치를 함께 반영하므로 더 선호한 영화를 상단에 배치하는 능력을 측정한다. Tie-aware Precision은 실제 평점순 K번째 영화의 평점을 cutoff로 사용하고, 같은 평점의 영화는 모두 동등하게 정답으로 인정해 Top-K 적중도를 측정한다. 현재 `final_score`는 순위 품질을 우선해 `0.8 × NDCG + 0.2 × Tie-aware Precision`으로 계산한다.
...
**최종 선택:** 사용자 기록을 시간순으로 나눈 `train 70% / holdout 30%`와 고정 holdout 후보 ranking을 사용한다. 70%는 취향 프로필을 구성할 학습 기록을 충분히 보존하고, 30%는 사용자별 NDCG와 Tie-aware Precision을 계산할 평가 영화를 확보하기 위한 절충이다. 시간순 분할로 미래 기록의 학습 유입을 막고, 모든 엔진에 동일한 holdout 후보를 제공해 후보 탐색량이 아닌 개인화 점수화·정렬 성능을 비교한다. 전체 카탈로그 retrieval 성능은 현재 점수와 섞지 않고 별도 평가로 확장한다.
...
- `Tie-aware Precision@20%`: 실제 평점순 K번째 평점을 cutoff로 정하고, 추천 Top-K 중 `rating >= cutoff`인 영화의 비율이다. cutoff와 동점인 영화는 모두 정답으로 인정하며 분모는 K이므로 완벽한 Top-K는 1.0이다.
- `NDCG@20%`: 평점에 등급을 부여해 더 좋아한 영화를 위에 배치했는지 측정한다.
  ...
  - 5.0점 → relevance 4
  - 3.0점 이하 → relevance 0
- `final_score = 0.8 × NDCG + 0.2 × Tie-aware Precision`

NDCG 비중을 높인 이유는 이 평가의 핵심이 단순 포함 여부보다 **사용자가 더 선호한 영화를 더 위에 배치하는 능력**이기 때문이다. Tie-aware Precision은 경계 평점 동점을 공정하게 처리하면서 Top-K 적중도를 보조 반영한다. 이 체제는 결과의 `metric_protocol.version = fixed-holdout-ranking-v2`로 기록하며, Recall을 사용한 과거 점수와 직접 비교하지 않는다.
...
`fixed-v1`과 지표 프로토콜 `fixed-holdout-ranking-v2`의 현재 회귀 기준 final score 평균은 다음과 같다.

| 엔진 | 10 | 50 | 100 | 150 | 200 | 500 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| V1 | 0.3738388882 | 0.4036924884 | 0.3444869766 | 0.3621781281 | 0.3530949363 | 0.3691667174 |
| V2 | 0.4327852801 | 0.3645774515 | 0.3329543722 | 0.3388727775 | 0.3330259235 | 0.3466340100 |
| V3 | 0.4724878217 | 0.4441785057 | 0.3630647459 | 0.3670169204 | 0.3547603701 | 0.3810528075 |
| V4 | 0.4074445465 | 0.4479121626 | 0.4393868997 | 0.4385201439 | 0.4402493013 | 0.4671954241 |
V1 원점수 활용과 V3 favorite+pin 등 기존 행동 표현 차이는 별도 정합성 문제이며,
현재 V1~V3와 학습 정보까지 완전히 통일됐다고 해석하지 않는다.
...
CPU 병렬화가 안전한 엔진은 다음 두 선택 계약을 추가할 수 있다.

- `rank_many(inputs)`: `(user_id, ranking)`을 반환하거나 yield한다. 공통 evaluator는 이 메서드가 있으면 사용자별 `rank` thread pool 대신 사용한다.
- `parallel_cohort_workers = 2`: 서로 독립인 cohort 모델을 최대 두 개 동시에 준비한다. DB 임시 데이터 namespace, 모델 파일과 캐시는 cohort별로 분리해야 한다.
- `configure_cohort_execution(namespace=...)`: 병렬 cohort가 파일·DB 식별자를 충돌 없이 분리해야 할 때 evaluator가 전달하는 namespace를 받는다.

이 선택 계약은 학습 입력·후보·점수 공식을 바꾸지 않는다. 엔진이 전역 mutable state,
공유 임시 테이블 또는 같은 모델 경로를 사용한다면 먼저 기본값인 cohort 직렬 실행으로
정확성을 확인한다. 병렬 버전은 같은 seed에서 사용자별 ranking 전체가 직렬 버전과 같아야 한다.
...
| `evaluation/benchmark.py` | cohort 반복, 병렬 사용자 평가, NDCG/Tie-aware Precision, 진행도, JSON·CSV 저장 | 평가의 핵심 실행기. 삭제 대상이 아님 |

...
Maintain and improve recommendation V3 as an independent engine without removing V1 or V2. The S0-S9 baseline and quality Phases A-I are complete; select new work from `z_v3_docs/08_additional_work_backlog.md`.
...
Current correction: candidate materialization stores top-150 per user as 100 active plus 50 ordered reserves, while detailed ontology analysis and policy reranking remain capped at 100. Feed-session continuity remains deferred in document 08. Phase I corrected synthetic-seed overlap, capped popular-item and high-activity-user training influence, reduced user identity dominance, and added population/user collaborative confidence. Confidence scales only the LightFM user-identity contribution; it must not reduce the whole LightFM candidate lane or transfer its weight to ontology. For known users, model/ontology candidate selection starts at 65/35 when top-50 movie overlap is zero and moves at most to 55/45 as overlap rises. Candidate selection is not reused as the final personal score; final personal uses model/short-term scores and detailed ontology is added once. The active model is `hybrid-0088e46c78e6-8ee76ae0cc79-51c5cf5ab1c1-32da0f11b507-16fe5319a5c2-7b869d3b`, candidate snapshot is `cand-72e516bd88d5e91a4a67ae1a`, and bundle is `bundle-c43cac19fbffc33c399f75b9`.

Long-term preference-ranking tuning now defaults known-user score centering to `0.5` and saved/pinned training half-lives to `365` days. The earlier focused-user experiment selected `730` days, but a follow-up cohort with 100 stable-focused, 100 stable-diverse, and 100 changing-preference users selected `365` days overall; type-specific winners differed, so this remains a provisional global compromise. The active 620-user model reached NDCG@20 `0.522051` and NDCG@30 `0.587833` on the fixed focused 100-user holdout, above the pure-LightFM baseline `0.513805/0.578975`. Hybrid artifact format 2 stores per-user onboarding-only signatures separately from long-term semantic features; known users must not be routed to cold start merely because their learned row contains long-term behavior. Multi-scale and repeated-seed confirmation remain open before declaring the preference-ranking stage complete.
...
Quality Phases A-I are complete. Phase I's unadjusted model reduced representative LightFM top-20 pairwise Jaccard from 21.2% to 9.8% while preserving stable genre overlap at 85.8%. The 120-user synthetic model has population confidence `0.1556`; this attenuates only the learned user-identity component during LightFM scoring. Corrected integrated validation produced 310 unique movies in 480 final slots and 90 unique movies in 120 top-5 slots. Target-genre top-5 matches were stable 30/30, drift-current 30/30, negative-heavy 30/30, and mixed 29/30, with zero exclusion or within-user duplicate violations. Base and effective model weights stayed equal. Long-term catalog qualification is now split from request-time filtering: static catalog and mature cold-item trust run before LightFM/ontology Top-K, while watched/passed, session, blacklist, and OTT remain request conditions. Four long-term user types retained 100 final candidates with no held-out positive rejected; short-term catalog-trust validation remains. Reports include `tests/v3_long_term_eval/outputs/h2_cross_validation/*_split_filter.json`. Actual-user relevance and population calibration remain unproven. Operations, traffic optimization, and auxiliary policy work are deferred while recommendation quality is the user priority. The latest V3 unit-test count is 142, plus 2 shared recommendation-executor tests.
